...
def bestalig(s1, s2, l1, l2):
	"""finds all the best possible alignments"""
	...
	with open('../Results/bestalig.txt', 'w+') as f:
		for item in my_best_align:
			f.write("%s\n" % item)
			f.write("                %s\n" % s1)
		f.write("Alignment score: %d" % my_best_score)
	
	print("Done! You will find your aligned sequences at '../Results/bestaligs.txt'")
	return 0
...
def main(argv):
	...
	if len(argv) == 1:
		...
		if argv[1:2].endswith('.fasta'):
			seq1 = read_fasta(argv[1])
			seq2 = read_fasta(argv[2])
			s = mulordseq(seq1, seq2)
			bestalig(s[0], s[1], s[2], s[3])
		else:
